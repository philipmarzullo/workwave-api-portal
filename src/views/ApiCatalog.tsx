...
type PlatformKey = 'winteam' | 'pestpac' | 'realgreen' | 'route_manager' | 'lighthouse' | 'timegate_plus'
...
export function ApiCatalog() {
  // Platform selector
  const [activePlatform, setActivePlatform] = useState<PlatformKey>('winteam')

  // Filters (for WinTeam catalog)
    ...
    <div className="py-8 space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display font-bold text-xl text-ww-navy">API Catalog</h1>
            <p className="text-sm text-ww-gray-500 mt-0.5">
              Browse endpoint inventories across WorkWave platforms
            </p>
          </div>
        </div>
            ...
            className={`relative flex items-center gap-2 px-4 py-2.5 rounded-lg text-[13px] font-medium transition-all whitespace-nowrap ${
              activePlatform === p.key
                ...
                : p.available
                  ...
                  : 'bg-ww-gray-50 border border-ww-gray-100 text-ww-gray-400 cursor-default'
            }`}
          >
            {p.label}
            {p.available ? (
      ...
      {/* Render active platform or coming-soon view */}
      {currentPlatform.available ? (
        <>
          {/* Platform sub-header */}
          <div className="flex items-center gap-2 text-[12px] text-ww-gray-500">
            <span className="font-semibold text-ww-navy">{currentPlatform.label}</span>
        ...
        /* Coming-soon view for unavailable platforms */
        <PlatformComingSoon platform={currentPlatform} />
      )}
    </div>
  )
}
...
function PlatformComingSoon({ platform }: { platform: PlatformDef }) {
  return (
    <div className="rounded-lg border border-dashed border-ww-gray-200 bg-ww-gray-50/50 py-16 px-6">
      <div className="max-w-md mx-auto text-center">
        <div className="w-12 h-12 rounded-xl bg-ww-gray-100 flex items-center justify-center mx-auto mb-4">
          <Lock size={20} className="text-ww-gray-400" />
        </div>
        <h2 className="text-lg font-display font-bold text-ww-navy mb-2">
          {platform.label} API Catalog
        </h2>
        <p className="text-sm text-ww-gray-500 mb-1">
          {platform.description}
        </p>
        <p className="text-sm text-ww-gray-400 mb-6">
          Gateway: <span className="font-mono">{platform.gateway}</span>
        </p>
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-ww-gray-100 text-[13px] font-medium text-ww-gray-500">
          <BookOpen size={14} />
          Endpoint inventory not yet imported
        </div>
        <p className="text-[11px] text-ww-gray-400 mt-4 leading-relaxed">
          Once the {platform.label} endpoint inventory is cataloged, it will appear here with the same
          browsing, filtering, and agent capabilities available for WinTeam.
        </p>
      </div>
    </div>
  )
